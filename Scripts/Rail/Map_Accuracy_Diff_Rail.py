...
INFILE = 'E:/Transit-Casa-Alex/MUNI Rail/Output/Modeling/Diff_Mapping_Data.csv'
OUTFILE = 'E:/Transit-Casa-Alex/MUNI Rail/Output/Modeling/Accuracy_Diff_Map.html'
...
if __name__ == "__main__":
    ...
    df['MOD_MOD_DIFF_DIFF'] = np.absolute(df['DIFF_AVG_RIDE_09']) - np.absolute(df['DIFF_AVG_RIDE_16'])
    df['MOD_MOD_DIFF_PDIFF'] = (df['MOD_MOD_DIFF_DIFF']/np.absolute(df['DIFF_AVG_RIDE_09']))*100 
    df['COLUMN_PER_STR'] = df['MOD_MOD_DIFF_PDIFF'].apply(lambda x : str(x) + '%')
    print('Mapping Your Differences!')
    
    mapa = map('STOP_ID','DIFF_AVG_RIDE_09','DIFF_AVG_RIDE_16','MOD_MOD_DIFF_PDIFF','COLUMN_PER_STR','MOD_MOD_DIFF_DIFF',df,color,radius,'LAT','LON')
